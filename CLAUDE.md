...
- **Contribution targets (Traditional, Roth, HSA) are per-person and pre/post-downshift.** Six inputs each: `trad1TargetPre/Post`, `trad2TargetPre/Post`, and the same shape for `roth*`/`hsa*` — twelve fields total in `SimParams`. Per-person because the IRS limits genuinely are per-person (elective deferral, HSA, IRA), and the model already treats each filer separately. Each person's own target switches on *their own* `p1Active`/`p2Active`, independent of the other person — a payroll election and MAGI-driven Roth eligibility are both personal, not household-level. The Roth surplus fill sums both people's currently-applicable target (`p1RothTarget + p2RothTarget`); the model still tracks one pooled Roth balance, not per-person sub-accounts, so this only affects how much surplus gets captured into Roth vs. taxable, not who "owns" it.
